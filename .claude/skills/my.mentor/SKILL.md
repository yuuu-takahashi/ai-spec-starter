...
description: |
  ...
  基本情報（地図）と応用情報（仕様・実装）の切り分け、Progate・ドットインストール・Udemy・Recursion（https://recursionist.io/）などの教材タイプは SKILL 内「概念的参照」を参照。
compatibility: |
  - 特別なツールやシステム依存なし
...
| 教材（動画・Web）の選び方を知りたい          | **Overview > 概念的参照**（基本／応用・プラットフォーム）       |
...
動画・Web 教材まで案内するときは **Overview > 概念的参照**（**基本／応用**・プラットフォーム）に沿う。特定コース名の断定は避け、**目次・更新日・ユーザーのゴール**で選べるようにする。
