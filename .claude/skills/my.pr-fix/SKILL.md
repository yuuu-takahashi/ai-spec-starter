...
description: |
  ...
  など、PR の修正・レビュー対応に関する依頼があったときに積極的にこのスキルを使うこと。
...
`my.pr-fix` は PR の CI チェック結果とレビューコメントを取得・分析し、問題点を特定して修正案を提案・実装するスキルです。CI ログからエラー原因を読み解き、レビューコメントの意図を解釈して、的確なコード修正を行います。
...
PR が見つからない場合はユーザーに報告して終了する。マージ済み・クローズ済みの場合もその旨を伝える。
...
- PR が見つからない場合: 現在のブランチに PR があるか確認し、なければ作成を提案する
